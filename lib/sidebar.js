...
var { Loader, Symbiont, Worker }    = require('api-utils/content'),
    { Trait }       = require('api-utils/traits'),
    observers       = require('api-utils/observer-service'),
    { URL }         = require('api-utils/url'),
    { sandbox, evaluate, load }     = require('api-utils/sandbox'),
...
var Sidebar = Trait.compose(
    ...
    constructor: function(options) {
         ...
         */
        self._initSymbiont( self.options );
    ...
    _onInit: function() {
        ...
        self._onSymbiontInit();

        /* :TODO: Traverse all <script> elements and load them
         *
         *        In the mean time, require scripts: [ ... ] in our
         *        options to load scripts.
         *
        if (self.options.scripts)
        {
            var sandbox = self._contentWorker.sandbox;

            self.options.scripts.forEach(function(url) {
                try {
                    var uri = URL(url);
console.log("Sidebar::_onInit(): load script[", url, "] into sandbox[",
                sandbox, "]");
                    ...
                    console.log("Sidebar::_onInit: Cannot load script[",
                                    url, "]:", e);
                }
            });
        }
